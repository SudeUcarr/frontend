...
<header className="landing-nav"><Brand/><nav><button onClick={()=>document.getElementById('araclar')?.scrollIntoView({behavior:'smooth'})}>Neler var?</button><button onClick={()=>document.getElementById('nasil')?.scrollIntoView({behavior:'smooth'})}>Nasıl çalışır?</button></nav><div className="landing-nav-actions"><ThemeControl inline/><Link className="button small" to="/giris">Giriş yap <ArrowUpRight size={16}/></Link></div></header>
